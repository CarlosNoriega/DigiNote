...
var datos="1";
  ...
  function added(){
    ...
    var ss=new Date();
    var s=ss.getTime();
    firebase.database().ref('users/' + userId+'/amigos/'+materia+'/chat/'+s).set({
      name : elname,
      mensaje : mensaje,
      id : userId
    });
    firebase.database().ref('users/' + materia+'/amigos/'+userId+'/chat/'+s).set({
      name : elname,
      mensaje : mensaje,
      id : userId
    });
  ...
  function cambiarnota(nam){
    ...
    firebase.auth().onAuthStateChanged(function(user) {
      ...
      if (user) {
        ...
        const usname=dbRefObject.child(userId);
        const cha=usname.child('amigos');
        const use=cha.child(nam);
        const dd=use.child('chat');
        dd.on('value', snapshot => {
          var dat=snapshot.val();
          console.log("mensaje"+dat);
  ...
  function initApp() {
    ...
    firebase.auth().onAuthStateChanged(function(user) {
      if (user) {
        ...
        const not=us.child('amigos');
        const pr=not.child(materia);
